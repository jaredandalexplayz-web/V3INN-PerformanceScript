...
Action(
    SettingsPage,
    "Reset Performance",
    "Put the original visual settings back.",
    function()
        SetSafeVisualMode(false)
...
Action(
    SettingsPage,
    "Run Command Audit",
    "Open the audit page and scan client-side definitions.",
    function()
...
-- Environment Studio
-- Local environment presets live here so they do not interfere with the FPS settings.
-- It creates a fifth "Environment" tab in the V3INN menu.
--
-- These changes are local to the client.
-- No RemoteEvent calls.
-- No RemoteFunction calls.
-- No server-side map edits.
-- Does not modify other players.
--
-- Available presets:
--   Default
--   Night Blue
...
Notify(
    "Command Audit",
    "Run the audit manually to inspect client-side definitions.",
    "warning"
)
